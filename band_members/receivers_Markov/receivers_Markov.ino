...
#define MEMBER_ID      4    // 本receiver的编号，从1开始（用于发送ACK2, READY2）
...
String vote_history = "nn";  // 初始假设前两轮都未插入 reharm
    ...
    if (!has_sync) {
      ...
      if (msg.indexOf(',') != -1 && msg != current_chord + "," + next_chord && msg != current_chord + "," + next_chord + "T") {
        ...
        if (chord2.endsWith("T")) {
          vote_history += "n";
          if (vote_history.length() > 2) {
            vote_history = vote_history.substring(vote_history.length() - 2);
          }
          // 正常主和弦对
          chord2.remove(chord2.length() - 1);  // 移除T标记
          current_chord = chord1;
...
void sendVote() {
  // 投票依据：vote_history 的第一个字符（上轮状态）
  char last = vote_history.charAt(0);  // 上一轮
  int vote_prob = (last == 'y') ? 35 : 50;

  bool vote_yes = (random(100) < vote_prob);
  String vote_msg = String(vote_yes ? "VOTE_YES" : "VOTE_NO") + String(MEMBER_ID);
  sendIR(vote_msg);

  Serial.print("[Receiver] Sent vote (Markov): ");
  Serial.print(vote_msg);
  Serial.print(" | History: ");
  Serial.print(vote_history);
  Serial.print(" | YES probability: ");
  Serial.print(vote_prob);
  Serial.println("%");
}
