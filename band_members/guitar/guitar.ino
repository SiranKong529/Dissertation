...
bool playSdWav(const char* filepath, float seconds = 2.0) {
  ...
  if (strncmp(header.riff_id, "RIFF", 4) != 0 ||
    ...
    Serial.println("Unsupported WAV format or not a valid WAV file!");
    file.close();
    return false;
  }
  ...
  while (file.position() < file.size()) {
    if (file.read((uint8_t*)&data_chunk, sizeof(DataChunkHeader)) != sizeof(DataChunkHeader)) {
      Serial.println("Failed to read chunk header!");
      file.close();
      return false;
    }
...
void loop() {
  ...
  if (has_new_raw_i2c_data) {
    ...
    if (sep > 0) {
      ...
      Serial.println("Warning: Received I2C data without comma separator.");
    }
  }

  if (now - last_play_time > 2000) {
    String chord_to_play = play_next ? next_chord : current_chord;
    if (chord_to_play.length() > 0) {
      String filepath = "/" + chord_to_play + ".wav";
      M5.Display.clear(BLACK);
      M5.Display.setCursor(0,0);
      M5.Display.println("Guitar:");
